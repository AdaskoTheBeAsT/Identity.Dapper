...
#pragma warning disable MA0026 // Fix TODO comment
    ...
    public class UserManagerTestsSqlServer
        : IClassFixture<SqlServerDatabaseFixture>
    {
        ...
        public UserManagerTestsSqlServer(SqlServerDatabaseFixture databaseFixture)
        {
            ...
            _userManager =
                (UserManager<DapperIdentityUser>)_databaseFixture.TestServer.Host.Services.GetService(
                    typeof(UserManager<DapperIdentityUser>));
        }
        ...
        public async Task CanCreateUserWithEmptyUserNameAsync()
        {
            ...
            Assert.Contains(
                result.Errors,
                x => x.Code.Equals(new IdentityErrorDescriber().InvalidUserName(string.Empty).Code, StringComparison.OrdinalIgnoreCase));
            Assert.False(result.Succeeded);
        }
